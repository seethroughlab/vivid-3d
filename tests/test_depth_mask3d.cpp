...
struct HeadlessGpu {
    ...
    bool init() {
        ...
        dev_desc.label = vivid::to_sv("DepthMask3D Test Device");
        ...
        dev_desc.uncapturedErrorCallbackInfo.callback =
            [](WGPUDevice const*, WGPUErrorType type, WGPUStringView msg, void*, void*) {
                std::fprintf(stderr, "[depth_mask3d_test] WebGPU error (%d): %.*s\n",
...
int main() {
    ...
    std::string staging = "./.test_depth_mask3d_staging";
    std::filesystem::create_directories(staging);

    const char* dylibs[] = {
        "shape3d.dylib", "render_3d.dylib", "transform3d.dylib",
        "scene_merge.dylib", "depth_mask3d.dylib"
    };
    ...
    if (!all_dylibs) {
        skip("One or more dylibs not found — skipping DepthMask3D tests");
    ...
    check(registry.find("DepthMask3D") != nullptr, "DepthMask3D registered");

    std::fprintf(stderr, "\n=== GPU init ===\n");
    HeadlessGpu gpu;
    if (!gpu.init()) {
        skip("No GPU available — skipping DepthMask3D tests");
        std::filesystem::remove_all(staging);
        std::fprintf(stderr, "\n%s: %d failure(s), %d skipped\n",
                     failures == 0 ? "ALL PASSED" : "SOME FAILED", failures, skipped);
        return failures > 0 ? 1 : 0;
    ...
    // -----------------------------------------------------------------
    // GPU Test: DepthMask3D masks color by depth
    // Scene: Shape3D → Render3D (color+depth) → DepthMask3D
    // -----------------------------------------------------------------
    std::fprintf(stderr, "\n=== GPU Test: DepthMask3D ===\n");
    {
        constexpr uint32_t W = 128, H = 128;

        vivid::Graph g;
        g.add_node("s1", "Shape3D", {{"r", 0.9f}, {"g", 0.2f}, {"b", 0.2f}});
        g.add_node("r1", "Render3D");
        g.add_node("dm", "DepthMask3D", {
            {"threshold", 0.5f}, {"softness", 0.3f}, {"mode", 1.0f}
        });
        g.add_connection("s1", "scene", "r1", "scene");
        g.add_connection("r1", "texture", "dm", "color");
        g.add_connection("r1", "depth", "dm", "depth");

        vivid::Scheduler sched;
        check(sched.build(g, registry), "DepthMask3D scene build succeeds");
        sched.allocate_gpu_textures(gpu.device, W, H, kFormat, WGPUTextureUsage_CopySrc);

        tick_and_submit(sched, gpu, kFormat);

        auto* dm_node = sched.find_node_mut("dm");
        check(dm_node != nullptr, "DepthMask3D node found");

        if (dm_node && dm_node->gpu_texture) {
            auto pixels = readback_texture(gpu.device, gpu.queue,
                                            dm_node->gpu_texture, W, H);
            ...
            if (!pixels.empty()) {
                ...
                check(center_lum > 0.01f, "DepthMask3D output is non-black");
            }
        }
