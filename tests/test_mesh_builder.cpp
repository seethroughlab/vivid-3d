...
struct HeadlessGpu {
    ...
    bool init() {
        ...
        dev_desc.label = vivid::to_sv("MeshBuilder Test Device");
        ...
        dev_desc.uncapturedErrorCallbackInfo.callback =
            [](WGPUDevice const*, WGPUErrorType type, WGPUStringView msg, void*, void*) {
                std::fprintf(stderr, "[mesh_builder_test] WebGPU error (%d): %.*s\n",
...
int main() {
    ...
    // =====================================================================
    // GPU integration tests — require dylibs and a GPU adapter
    // =====================================================================

    std::string staging = "./.test_mesh_builder_staging";
    std::filesystem::create_directories(staging);

    const char* dylibs[] = {
        "mesh_builder.dylib", "render_3d.dylib"
    };
    ...
    check(registry.find("MeshBuilder") != nullptr, "MeshBuilder registered");
    ...
    // Primitive types: Box=0, Sphere=1, Cylinder=2, Cone=3, Torus=4, Plane=5
    const char* primitive_names[] = {
        "Box", "Sphere", "Cylinder", "Cone", "Torus", "Plane"
    };

    for (int prim_idx = 0; prim_idx < 6; ++prim_idx) {
        std::fprintf(stderr, "\n=== GPU Test: MeshBuilder primitive %s (%d) -> Render3D ===\n",
                     primitive_names[prim_idx], prim_idx);
        {
            constexpr uint32_t W = 64, H = 64;

            vivid::Graph g;
            g.add_node("mb", "MeshBuilder", {{"primitive", static_cast<float>(prim_idx)}});
            g.add_node("r1", "Render3D");
            g.add_connection("mb", "scene", "r1", "scene");
            ...
            if (!pixels.empty()) {
                ...
                std::snprintf(msg, sizeof(msg),
                              "center pixel is non-black (%s visible)", primitive_names[prim_idx]);
                check(rv > 0 || gv > 0 || bv > 0, msg);
            }
